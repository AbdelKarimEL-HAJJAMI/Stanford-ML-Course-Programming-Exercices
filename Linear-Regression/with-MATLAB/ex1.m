...
% to list the files in your Current Folder.
...
% The first column is the population of a city and the second column is the profit 
...
X = data(:, 1); y = data(:, 2);
%% 
% Your job is to complete |*plotData.m*| to draw the plot; modify the file and 
...
%
%% 
% Once you are finished, save |plotData.m,| and execute the code in this section 
% which will call |plotData|. 
...
% 
%% 
% To learn more about the |plot| command, you can run the command |*help plot| 
% at the command prompt, type |plot()| inside the MATLAB Live Editor and click 
% on the "(?)" tooltip, or you can search the <https://www.mathworks.com/help/matlab/ 
...
% 
% $$J\left(\theta \right)=\frac{1}{2m}\sum_{i=1}^m {\left(h_{\theta \;} \left(x^{\left(i\right)} 
% \right)-y^{\left(i\right)} \right)}^2 \;$$
% 
...
% the values you will adjust to minimize cost $J(\theta)$. One way to do this 
% is to use the batch gradient descent algorithm. In batch gradient descent, each 
% iteration performs the update
% 
% $$\theta_j :=\theta_j -\alpha \frac{\;1}{m}\sum_{i=1}^m \;\left(h_{\theta 
% \;} \left(x^{\left(i\right)} \right)-y^{\left(i\right)} \right)x_j^{\left(i\right)} 
% \;\;\;\;\;\;\;\left(\textrm{simultaneously}\;\textrm{update}\;\theta_{j\;} \;\textrm{for}\;\textrm{all}\;j\right)$$
% 
...
% is a function that computes $J\left(\theta \right)$. As you are doing this, 
% remember that the variables |X| and |y| are not scalar values, but matrices 
% whose rows represent the examples from the training set.
% 
% Once you have completed the function definition, run this section. The code 
% below will call |computeCost| once using $\theta$ initialized to zeros, and 
% you will see the cost printed to the screen. You should expect to see a cost 
% of 32.07 for the first output below:

% Compute and display initial cost with theta all zeros
computeCost(X, y, theta)
%% 
% Next we call |computeCost| again, this time with non-zero |theta| values as 
% an additional test. You should expect to see an output of 54.24 below:
...
% what is being updated. Keep in mind that the cost $J\left(\theta \right)$ is 
...
% 
% After you are finished, run this execute this section. The code below will 
% use your final parameters to plot the linear fit. The result should look something 
% like Figure 2 below:
% 
% 
% 
...
%% 
% Note the way that the lines above use matrix multiplication, rather than explicit 
% summation or looping, to calculate the predictions. This is an example of _code 
% vectorization_ in MATLAB. 
%% 
% _You should now submit your solutions.  Enter_ |*submit*| _at the command 
% prompt, then enter or confirm your login and token when prompted._
% 1.3 Debugging
...
% 
% In the next step, there is code set up to calculate $J(\theta)$ over a grid 
% of values using the |computeCost| function that you wrote. 
...
% changes in $\theta_0$ and $\theta_1$. The cost function $J(\theta)$ is bowl-shaped 
% and has a global mininum. (This is easier to see in the contour plot than in 
% the 3D surface plot). This minimum is the optimal point for $\theta_0$ and $\theta_1$, 
...
% Run this section now to preview the data.
...
%% 
% The remainder of this script has been set up to help you step through this 
% exercise.
...
%% 
% The standard deviation is a way of measuring how much variation there is in 
% the range of values of a particular feature (most data points will lie within 
...
% matrix |*X*| corresponds to one feature. When you are finished with |featureNormailize.m|, 
% run this section to normailze the features of the housing dataset.

% Scale features and set them to zero mean
[X, mu, sigma] = featureNormalize(X);
%% 
% *Implementation Note:* When normalizing the features, it is important to store 
% the values used for normalization - the mean value and the standard deviation 
% used for the computations. After learning the parameters from the model, we 
% often want to predict the prices of houses we have not seen before. Given a 
% new |x| value (living room area and number of bedrooms), we must first normalize 
% |x| using the mean and standard deviation that we had previously computed from 
% the training set.
% Add the bias term
% Now that we have normailzed the features, we again add a column of ones corresponding 
% to $\theta_0$ to the data matrix |X*.*|
...
% to implement the cost function and gradient descent for linear regression _with 
% multiple variables_. If your code in the previous part (single variable) already 
...
% in the dataset.
% 
% We have provided you with the following starter code below that runs gradient 
% descent with a particular learning rate (|alpha|). Your task is to first make 
% sure that your functions |computeCost| and |gradientDescent| already work with 
...
% where
% 
% $$X=\left\lbrack \begin{array}{c}{-\;\left(x^{\left(1\right)} \right)}^T -\\{-\;\left(x^{\left(2\right)} 
% \right)}^T -\\\vdots \;\\{-\;\left(x^{\left(m\right)} \right)}^T -\end{array}\right\rbrack 
% \;\;\;\;\;\;\;\;\;\;\;\vec{y} =\left\lbrack \begin{array}{c}y^{\left(1\right)} 
% \\y^{\left(2\right)} \\\vdots \;\\y^{\left(m\right)} \end{array}\right\rbrack$$
% 
% The vectorized version is efficient when you're working with numerical computing 
% tools like MATLAB. If you are an expert with matrix operations, you can prove 
% to yourself that the two forms are equivalent.
...
%% 
% Finally, you should complete and run the code below to predict the price of 
% a 1650 sq-ft, 3 br house using the value of |theta| obtained above. 
% 
% *Hint:* At prediction, make sure you do the same feature normalization. Recall 
% that the first column of |X| is all ones. Thus, it does not need to be normalized.

% Estimate the price of a 1650 sq-ft, 3 br house
% ====================== YOUR CODE HERE ======================

price = theta'*([1, ([1650,3]-mu)./sigma])' % Enter your price formula here
...
% 
% *Implementation Note:* If your learning rate is too large, $J(\theta)$ can 
% diverge and 'blow up', resulting in values which are too large for computer 
...
%
%% 
% The final arguments |'b'|,|'r'|, and |'k*'*| specify different colors for 
% the plots. If desired, you can use this technique and adapt the code below to 
% plot multiple convergence histories in the same plot. 
...
%% 
% Using the best learning rate that you found, run the section of code below, 
% which will run gradient descent until convergence to find the final values of 
% $\theta$. Next, use this value of $\theta$ to predict the price of a house with 
% 1650 square feet and 3 bedrooms. You will use value later to check your implementation 
% of the normal equations. Don't forget to normalize your features when you make 
% this prediction!
...
% then run the code in this section. Remember that while you don't need to scale 
% your features, we still need to add a column of 1's to the |X| matrix to have 
% an intercept term ($\theta_0$) . Note that the code below will add the column 
% of 1's to |X| for you.
% 
% 
